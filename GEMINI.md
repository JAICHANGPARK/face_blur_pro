...
This project is a Flutter application that provides face blurring functionality for images. It's a proof-of-concept that demonstrates a cross-platform approach, using different technologies for face detection on mobile and desktop platforms.

The application allows users to:
- Pick an image from their gallery.
- Detect faces in the image.
- Select which faces to blur.
- Apply a rectangular or circular blur to the selected faces.
- Save the modified image to the gallery or share it.

### Architecture

The application is built with Flutter and Dart for the UI and application logic. For face detection, it uses a hybrid approach:

- **Mobile (Android/iOS):** It uses the `google_mlkit_face_detection` package, which leverages the on-device ML capabilities of the mobile platforms.
- **Desktop (macOS/Linux/Windows):** It uses a Rust backend with the `tract-onnx` crate to perform face detection using a pre-trained ONNX model (`version-RFB-640.onnx`).

The image blurring is implemented in Rust for all platforms to ensure consistent results. The communication between the Flutter frontend and the Rust backend is handled by the `flutter_rust_bridge` library.

## Key Technologies

- **Frontend:** Flutter
- **Backend (for desktop face detection and blurring):** Rust
- **Face Detection (Mobile):** Google ML Kit
- **Face Detection (Desktop):** ONNX model (`version-RFB-640.onnx`)
- **Bridge:** `flutter_rust_bridge`
- **Image Processing (Rust):** `image`, `tract-onnx`
...
The application should build and run on your connected device or simulator.

## Development Conventions

### Code Structure

- **Flutter/Dart:** The main application logic is in `lib/main.dart`.
- **Rust:** The Rust code is located in the `rust` directory. The main logic is in `rust/src/api/simple.rs`.
- **Bridge:** The `flutter_rust_bridge.yaml` file configures the code generation for the bridge between Flutter and Rust. The generated code is located in `lib/src/rust`.

### Face Detection

Face detection is handled differently depending on the platform:

- In `lib/main.dart`, the `_pickAndDetect` function checks the platform.
- If it's Android or iOS, it uses `google_mlkit_face_detection`.
- If it's a desktop platform, it calls the `detect_faces_desktop` function in Rust, passing the image and model bytes.

### Face Blurring

The `blur_multiple_faces` function in `rust/src/api/simple.rs` is responsible for blurring the faces. It takes the image bytes, a list of rectangles to blur, and a boolean to indicate whether the blur should be circular. This function is called from the `_blurSelectedFaces` function in `lib/main.dart`.
